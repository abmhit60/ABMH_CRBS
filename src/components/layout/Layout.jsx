    ...
    <div className="app">
      <aside className="sidebar">
        ...
        <div className="sb-footer">
          ...
          <div className="u-info"><div className="u-name">{profile?.full_name}</div><div className="u-role">{profile?.role ?? 'staff'}</div></div>
        ...
        <>
          ...
          <div className="drawer">
            <div className="drawer-head">
              ...
              <div><div style={{ fontWeight: 700, fontSize: 15, color: '#fff' }}>{profile?.full_name}</div><div style={{ fontSize: 12, color: 'rgba(255,255,255,.4)' }}>{profile?.department_name ?? profile?.role}</div></div>
            </div>
            <nav className="drawer-nav"><NavList onClick={() => setDrawer(false)} /></nav>
            <div className="drawer-foot">
              <button onClick={logout} style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 10, padding: '12px 16px', background: 'rgba(220,38,38,.15)', color: '#f87171', border: '1px solid rgba(220,38,38,.2)', borderRadius: 10, fontSize: 14, fontWeight: 600, fontFamily: 'inherit', cursor: 'pointer' }}>
                {IC.out} Sign Out
              </button>
            </div>
          </div>
